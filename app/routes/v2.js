...
router.post('/responsible-answer-v2', function(request, response) {

  var responsible = request.session.data['responsibleforpaying']
  if (responsible == "yes"){
      response.redirect("/v2/eligibility/where-you-live")
  } else {
      response.redirect("/v2/eligibility/not-responsible")
  }
})

// WHERE YOU LIVE

router.post('/whereyoulive-answer-v2', function(request, response) {

  var country = request.session.data['whereyoulive']
  if (country == "abroad"){
    response.redirect("/v2/eligibility/live-outside-uk")
  } else if (country == "northern-ireland"){
        response.redirect("/v2/eligibility/live-in-ni")
  } else if (country == "scotland"){
        response.redirect("/v2/eligibility/live-in-scotland")
  } else {
      response.redirect("/v2/eligibility/benefits")
  }
})

  // BENEFITS ANSWER

  router.post('/benefits-answer-v2', function(request, response) {

    var benefits = request.session.data['benefits']
    if (benefits == "no"){
        response.redirect("/v2/eligibility/not-on-qb")
    } else {
        response.redirect("/v2/eligibility/usually-live-in-uk")
    }
})

  // DID THE DECEASED LIKE IN THE UK

  router.post('/deceaseduk-answer-v2', function(request, response) {

    var deceaseduk = request.session.data['deceaseduk']
    if (deceaseduk == "no"){
        response.redirect("/v2/eligibility/deceased-outside-uk")
    } else {
        response.redirect("/v2/eligibility/funeral-taken-place")
    }
})

  // FUNERAL IN UK

  router.post('/funeralinuk-answer-v2', function(request, response) {

    var funeralinuk = request.session.data['funeralinuk']
    if (funeralinuk == "no"){
        response.redirect("/v2/eligibility/funeral-in-eea")
    } else {
        response.redirect("/v2/eligibility/check-answers-eligibility")
    }
})

  // FUNERAL IN EEA

  router.post('/funeraleea-answer-v2', function(request, response) {

    var funeraleea = request.session.data['funeraleea']
    if (funeraleea == "no"){
        response.redirect("/v2/eligibility/funeral-outside-uk")
    } else {
        response.redirect("/v2/eligibility/check-answers-eligibility")
    }
})
  ...
  router.post('/financial-affairs-answer-v2', function(request, response) {

    var financialaffairs = request.session.data['financialaffairs']
    if (financialaffairs == "you"){
        response.redirect("/v2/money/bank-accounts")
    } else {
        response.redirect("/v2/money/responsible-for-finances-name")
    }
})

  // MONEY IN BANK ACCOUNTS

  router.post('/bankaccounts-answer-v2', function(request, response) {

    var bankaccounts = request.session.data['bankaccounts']
    if (bankaccounts == "yes"){
        response.redirect("/v2/money/exact-amount-bank-account")
    } else {
        response.redirect("/v2/money/isa")
    }
})

  // MONEY IN JOINT ACCOUNT

  router.post('/jointaccount-answer-v2', function(request, response) {

    var jointaccount = request.session.data['jointaccount']
    if (jointaccount == "yes"){
        response.redirect("/v2/money/exact-amount-joint-account")
    } else if (jointaccount == "all"){
        response.redirect("/v2/money/partner-joint-account")    
    } else {
        response.redirect("/v2/money/isa")
    }
})
  ...
  router.post('/partnerjointaccount-answer-v2', function(request, response) {

    var partnerjointaccount = request.session.data['partnerjointaccount']
    if (partnerjointaccount == "no"){
        response.redirect("/v2/money/joint-account-transfer")
    } else {
        response.redirect("/v2/money/isa")
    }
})
  ...
  router.post('/jointaccounttransfer-answer-v2', function(request, response) {

    var jointaccounttransfer = request.session.data['jointaccounttransfer']
    if (jointaccounttransfer == "yes"){
        response.redirect("/v2/money/exact-amount-other-joint-account")
    } else {
        response.redirect("/v2/money/isa")
    }
})
  ...
  router.post('/isas-answer-v2', function(request, response) {

    var isas = request.session.data['isas']
    if (isas == "yes"){
        response.redirect("/v2/money/exact-amount-isa")
    } else {
        response.redirect("/v2/money/in-cash")
    }
})

  // MONEY IN CASH

  router.post('/cash-answer-v2', function(request, response) {

    var cash = request.session.data['cash']
    if (cash == "yes"){
        response.redirect("/v2/money/exact-amount-cash")
    } else {
        response.redirect("/v2/money/plans-filter")
    }
})

  // PLANS FILTER QUESTION

  router.post('/plansfilter-answer-v2', function(request, response) {

    var plansfilter = request.session.data['plansfilter']
    if (plansfilter == "no"){
        response.redirect("/v2/money/burial-club")
    } else {
        response.redirect("/v2/money/workplace-pension")
    }
})

  // WORKPLACE PENSION

  router.post('/pension-answer-v2', function(request, response) {

    var pension = request.session.data['pension']
    if (pension == "yes"){
        response.redirect("/v2/money/exact-amount-pension")
    } else {
        response.redirect("/v2/money/life-insurance")
    }
})

  // LIFE INSURANCE

  router.post('/lifeinsurance-answer-v2', function(request, response) {

    var lifeinsurance = request.session.data['lifeinsurance']
    if (lifeinsurance == "yes"){
        response.redirect("/v2/money/exact-amount-insurance")
    } else {
        response.redirect("/v2/money/funeral-plan")
    }
})

  // FUNERAL PLAN

  router.post('/funeralplan-answer-v2', function(request, response) {

    var funeralplan = request.session.data['funeralplan']
    if (funeralplan == "yes"){
        response.redirect("/v2/money/funeral-plan-cover-costs")
    } else {
        response.redirect("/v2/money/armed-forces")
    }
})

  // FUNERAL PLAN COVER ANY OF THE FUNERAL COSTS

  router.post('/funeralplancover-answer-v2', function(request, response) {

    var funeralplancover = request.session.data['funeralplancover']
    if (funeralplancover == "no"){
        response.redirect("/v2/money/funeral-plan-refund")
    } else {
        response.redirect("/v2/money/120-limit")
    }
})

  // FUNERAL PLAN REFUND

  router.post('/funeralplanrefund-answer-v2', function(request, response) {

    var funeralplanrefund = request.session.data['funeralplanrefund']
    if (funeralplanrefund == "yes"){
        response.redirect("/v2/money/exact-amount-funeral-refund")
    } else {
        response.redirect("/v2/money/armed-forces")
    }
})
  ...
  router.post('/burialclub-answer-v2', function(request, response) {

    var burialclub = request.session.data['burialclub']
    if (burialclub == "yes"){
        response.redirect("/v2/money/exact-amount-burial-club")
    } else {
        response.redirect("/v2/money/other-money")
    }
})

  // AMRED FORCES

  router.post('/armedforces-answer-v2', function(request, response) {

    var armedforces = request.session.data['armedforces']
    if (armedforces == "yes"){
        response.redirect("/v2/money/war-pension")
    } else {
        response.redirect("/v2/money/burial-club")
    }
})
  ...
  router.post('/warpension-answer-v2', function(request, response) {

    var warpension = request.session.data['warpension']
    if (warpension == "yes"){
        response.redirect("/v2/money/exact-amount-war-pension")
    } else {
        response.redirect("/v2/money/burial-club")
    }
})
  ...
  router.post('/othermoney-answer-v2', function(request, response) {

    var othermoney = request.session.data['othermoney']
    if (othermoney == "yes"){
        response.redirect("/v2/money/exact-amount-other-money")
    } else {
        response.redirect("/v2/money/death-certificates")
    }
})

  // PERMISSION TO COLLECT

  router.post('/permission-to-collect-v2', function(request, response) {

    var permission = request.session.data['permission']
    if (permission == "yes"){
        response.redirect("/v2/money/check-answers-money")
    } else {
        response.redirect("/v2/money/permission-to-collect-in-future")
    }
})

  // FUNERAL DIRECTOR

  router.post('/director-answer-v2', function(request, response) {

    var director = request.session.data['director']
    if (director == "yes"){
        response.redirect("/v2/funeral/provider-name")
    } else {
        response.redirect("/v2/funeral/someone-else")
    }
})

  // SOMEONE ELSE ARRANGING 

  router.post('/someoneelse-answer-v2', function(request, response) {

    var someoneelse = request.session.data['someoneelse']
    if (someoneelse == "yes"){
        response.redirect("/v2/funeral/provider-name")
    } else {
        response.redirect("/v2/funeral/have-you-used-money-from-deceased")
    }
})

   // HAVE YOU USED MONEY FROM DECEASED TO PAY FUNERAL (asked if arranging yourself)

   router.post('/haveyouusedmoney-answer-v2', function(request, response) {

    var haveyouusedmoney = request.session.data['haveyouusedmoney']
    if (haveyouusedmoney == "yes"){
        response.redirect("/v2/funeral/exact-amount-money-used")
    } else {
        response.redirect("/v2/funeral/claim-travel")
    }
})

  // DO YOU HAVE A FUNERAL BILL OR SIGNED CONTRACT YET

  router.post('/funeralbill-answer-v2', function(request, response) {

    var funeralbill = request.session.data['funeralbill']
    if (funeralbill == "yes"){
        response.redirect("/v2/funeral/bill-in-your-name")
    } else {
        response.redirect("/v2/funeral/will-bill-be-in-your-name")
    }
})

 // BILL IN YOUR NAME 

router.post('/billinyourname-answer-v2', function(request, response) {

    var billinyourname = request.session.data['billinyourname']
    if (billinyourname == "no"){
        response.redirect("/v2/funeral/arranged-on-your-behalf")    
    } else {
        response.redirect("/v2/funeral/money-paid-off-bill")
    }
  })

     // PAID MONEY OFF BILL

router.post('/moneyoffbill-answer-v2', function(request, response) {

    var moneyoffbill = request.session.data['moneyoffbill']
    if (moneyoffbill == "yes"){
        response.redirect("/v2/funeral/exact-amount-money-off-bill")
    } else {
        response.redirect("/v2/funeral/items-not-on-bill")
    }
})

   // USED MONEY FROM DECEASED TO PAY FUNERAL

router.post('/usedmoney-answer-v2', function(request, response) {

    var usedmoney = request.session.data['usedmoney']
    if (usedmoney == "yes"){
        response.redirect("/v2/funeral/exact-amount-used-money")
    } else {
        response.redirect("/v2/funeral/items-not-on-bill")
    }
})
   ...
   router.post('/claimtravel-answer-v2', function(request, response) {

    var claimtravel = request.session.data['claimtravel']
    if (claimtravel == "yes"){
        response.redirect("/v2/funeral/travel-expenses")
    } else {
        response.redirect("/v2/funeral/check-answers-funeral")
    }
})

   // DOCUMENTS ANSWER

router.post('/documents-answer-v2', function(request, response) {
   ...
   router.post('/arrangementsonyourbehalf-answer-v2', function(request, response) {

    var arrangementsonyourbehalf = request.session.data['arrangementsonyourbehalf']
    if (arrangementsonyourbehalf == "yes"){
        response.redirect("/v2/funeral/money-paid-off-bill")
    } else {
        response.redirect("/v2/funeral/no-permission")
    }
})

   // NO PERMISSION GIVEN (WHEN CITIZEN HAS BILL OR SIGNED CONTRACT)

   router.post('/permission-answer-v2', function(request, response) {
   ...
   router.post('/permission-other-answer-v2', function(request, response) {

    var nopermissionother = request.session.data['nopermissionother']
    if (nopermissionother == "yes"){
        response.redirect("/v2/funeral/claim-travel-in-future")
    } else {
        response.redirect("/index")
    }
})
 ...
 router.post('/willbillbeinyourname-answer-v2', function(request, response) {

    var willbillbeinyourname = request.session.data['willbillbeinyourname']
    if (willbillbeinyourname == "no"){
        response.redirect("/v2/funeral/allowing-on-your-behalf")    
    } else {
        response.redirect("/v2/funeral/claim-travel-in-future")
    }
  })
 ...
 router.post('/allowingonyourbehalf-answer-v2', function(request, response) {
...
// WHAT TYPE OF TRAVEL ROUTES
  ...
  router.post('/transporttype-answer-v2', function(request, response) {

    var transporttype = request.session.data['transporttype']
    if (transporttype.includes("car")){
        response.redirect("/v2/funeral/check-answers-funeral")
    } else {
        response.redirect("/v2/funeral/how-much-paid")
    }
})
...
router.post('/over-20-answer', function(request, response) {
    ...
    if (over20.includes("yes")){
        response.redirect("/v2/eligibility/partner")
    } else {
        response.redirect("/v2/eligibility/under16")
    }
})
...
router.post('/16to19-answer', function(request, response) {
    ...
    if (under16.includes("no")){
        ...
        response.redirect("/v2/eligibility/cannot-use-private-beta")
    }
})

// ARE YOU THE PARTNER

router.post('/partner-answer-v2', function(request, response) {

    var relationship = request.session.data['partner']
    if (relationship == "yes"){
        response.redirect("/v2/eligibility/same-address")
    } else {
        response.redirect("/v2/eligibility/do-you-have-partner")
    }
  })

  // WERE YOU LIVING TOGETHER

router.post('/same-address-answer-v2', function(request, response) {

    var address = request.session.data['sameaddress']
    if (address == "no"){
        response.redirect("/v2/eligibility/cr-other-ifms")
    } else {
        response.redirect("/v2/eligibility/responsible-for-paying")
    }
  })
    ...
    router.post('/cr-other-ifms-answer', function(request, response) {
        ...
        if (ifm.includes("none")){
            ...
            response.redirect("/v2/eligibility/ifm-should-apply")
        }
    })
    ...
    // IFM SOFT STOP
        ...
        router.post('/deceasedhavepartner-answer', function(request, response) {
            ...
            if (deceasedhavepartner == "yes"){
                response.redirect("/v2/eligibility/partner-should-apply")
            } else {
                response.redirect("/v2/eligibility/relationship-to-deceased")
            }
          })
        ...
        router.post('/relationship-to-deceased-answer', function(request, response) {
            ...
            if (relationshiptodeceased == "childofdeceased"){
              response.redirect("/v2/eligibility/child-other-ifms")
            } else if (relationshiptodeceased == "parentofdeceased"){
                  response.redirect("/v2/eligibility/parent-other-ifms")
            } else {
                response.redirect("/v2/eligibility/cr-other-ifms")
            }
          })
        ...
        router.post('/relationshipbrokendown-answer', function(request, response) {
            ...
            if (relationshipbrokendown == "yes"){
              response.redirect("/v2/ifm/parent/add-another-parent")
            } else {
                response.redirect("/v2/ifm/parent/their-details")
            }
          })    
          ...
          router.post('/parent-qb-answer', function(request, response) {
            ...
            if (parentonqb == "Yes"){
              response.redirect("/v2/ifm/parent/benefits")
            } else if (parentonqb == "dontknow"){
                  response.redirect("/v2/ifm/parent/add-another-parent")
            } else {
                response.redirect("/v2/ifm/parent/exemption-list")
            }
          })
        ...
        router.post('/what-benefits-parent-answer', function(request, response) {
            ...
            if (whatbenefitsparent == "no"){
              response.redirect("/v2/ifm/parent/soft-stop")
            } else {
                response.redirect("/v2/ifm/parent/add-another-parent")
            }
          })
        ...
        router.post('/parent-exemption-answer', function(request, response) {
         ...
         // ADDING SECOND PARENT
          ...
          router.post('/add-2nd-parent-answer', function(request, response) {
            ...
            if (parenttwo == "no"){
              response.redirect("/v2/ifm/child/about-a-child")
            } else {
                response.redirect("/v2/ifm/parent2/about-a-parent")
            }
          })  
          ...
          router.post('/add-2nd-parent-no-child-answer', function(request, response) {
            ...
            var parenttwo = request.session.data['parenttwo']
            if (parenttwo == "no"){
              response.redirect("/v2/money/start")
            } else {
                response.redirect("/v2/ifm/parent2/about-a-parent")
            }
          })  
          ...
          router.post('/relationshipbrokendown-answer2', function(request, response) {
            ...
            if (relationshipbrokendown2 == "yes"){
              response.redirect("/v2/ifm/parent2/add-another-parent")
            } else {
                response.redirect("/v2/ifm/parent2/their-details")
            }
          })    
          ...
          router.post('/parent-qb-answer2', function(request, response) {
            ...
            if (parentonqb2 == "Yes"){
              response.redirect("/v2/ifm/parent2/benefits")
            } else if (parentonqb2 == "dontknow"){
                  response.redirect("/v2/ifm/parent2/add-another-parent")
            } else {
                response.redirect("/v2/ifm/parent2/exemption-list")
            }
          })
        ...
        router.post('/what-benefits-parent-answer2', function(request, response) {
            ...
            if (whatbenefitsparent2 == "no"){
              response.redirect("/v2/ifm/parent2/soft-stop")
            } else {
                response.redirect("/v2/ifm/parent2/add-another-parent")
            }
          })
        ...
        router.post('/relationshipbrokendownc1-answer', function(request, response) {
            ...
            if (relationshipbrokendownc1 == "yes"){
              response.redirect("/v2/ifm/child/add-another-child")
            } else {
                response.redirect("/v2/ifm/child/their-details")
            }
          })    
         ...
         router.post('/child-qb-answer', function(request, response) {
            ...
            if (childonqb == "Yes"){
              response.redirect("/v2/ifm/child/benefits")
            } else if (childonqb == "dontknow"){
                  response.redirect("/v2/ifm/child/add-another-child")
            } else {
                response.redirect("/v2/ifm/child/exemption-list")
            }
          }) 
        ...
        router.post('/what-benefits-child-answer', function(request, response) {
            ...
            if (whatbenefitschild == "no"){
              response.redirect("/v2/ifm/child/soft-stop")
            } else {
                response.redirect("/v2/ifm/child/add-another-child")
            }
          })
          ...
          router.post('/add-2nd-child-answer', function(request, response) {
            ...
            if (childtwo == "no"){
              response.redirect("/v2/money/start")
                ...
                response.redirect("/v2/ifm/child2/about-a-child")
            }
          })  
        ...
        router.post('/relationshipbrokendownc2-answer', function(request, response) {
            ...
            if (relationshipbrokendownc2 == "yes"){
              response.redirect("/v2/ifm/child2/add-another-child")
            } else {
                response.redirect("/v2/ifm/child2/their-details")
            }
          })    
         ...
         router.post('/child2-qb-answer', function(request, response) {
            ...
            if (childonqb2 == "Yes"){
              response.redirect("/v2/ifm/child2/benefits")
            } else if (childonqb2 == "dontknow"){
                  response.redirect("/v2/ifm/child2/add-another-child")
            } else {
                response.redirect("/v2/ifm/child2/exemption-list")
            }
          }) 
        ...
        router.post('/what-benefits-child-answer2', function(request, response) {
            ...
            if (whatbenefitschild2 == "no"){
              response.redirect("/v2/ifm/child2/soft-stop")
            } else {
                response.redirect("/v2/ifm/child2/add-another-child")
            }
          })
          ...
          router.post('/add-3rd-child-answer', function(request, response) {
            ...
            if (childthree == "no"){
              response.redirect("/v2/money/start")
                ...
                response.redirect("/v2/ifm/child3/about-a-child")
            }
          })  
        ...
        router.post('/relationshipbrokendownc3-answer', function(request, response) {
            ...
            if (relationshipbrokendownc3 == "yes"){
              response.redirect("/v2/ifm/child3/add-another-child")
            } else {
                response.redirect("/v2/ifm/child3/their-details")
            }
          })    
         ...
         router.post('/child3-qb-answer', function(request, response) {
            ...
            if (childonqb3 == "Yes"){
              response.redirect("/v2/ifm/child3/benefits")
            } else if (childonqb3 == "dontknow"){
                  response.redirect("/v2/ifm/child3/add-another-child")
            } else {
                response.redirect("/v2/ifm/child3/exemption-list")
            }
          }) 
        ...
        router.post('/what-benefits-child-answer3', function(request, response) {
            ...
            if (whatbenefitschild3 == "no"){
              response.redirect("/v2/ifm/child3/soft-stop")
            } else {
                response.redirect("/v2/ifm/child3/add-another-child")
            }
          })
          ...
          router.post('/add-4th-child-answer', function(request, response) {
            ...
            if (childfour == "no"){
              response.redirect("/v2/money/start")
                ...
                response.redirect("/v2/ifm/child4/about-a-child")
            }
          })  
        ...
        router.post('/relationshipbrokendownc4-answer', function(request, response) {
            ...
            if (relationshipbrokendownc4 == "yes"){
              response.redirect("/v2/ifm/child4/add-another-child")
            } else {
                response.redirect("/v2/ifm/child4/their-details")
            }
          })    
         ...
         router.post('/child4-qb-answer', function(request, response) {
            ...
            if (childonqb4 == "Yes"){
              response.redirect("/v2/ifm/child4/benefits")
            } else if (childonqb4 == "dontknow"){
                  response.redirect("/v2/ifm/child4/add-another-child")
            } else {
                response.redirect("/v2/ifm/child4/exemption-list")
            }
          }) 
        ...
        router.post('/what-benefits-child-answer4', function(request, response) {
            ...
            if (whatbenefitschild4 == "no"){
              response.redirect("/v2/ifm/child4/soft-stop")
            } else {
                response.redirect("/v2/ifm/child4/add-another-child")
            }
          })
